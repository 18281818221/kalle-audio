...
def vae_sample(mean, scale):
        ...
        latents = torch.randn_like(mean) * scale + mean
        # latents = mean + torch.randn_like(mean) * torch.exp(scale) # [B, 64, T] standard inference pipeline for tts
        print(latents.shape, torch.mean(mean), torch.mean(stdev), torch.mean(scale))
...
class VAEBottleneck(Bottleneck): # inference time
    def __init__(self):
        super().__init__(is_discrete=False)
