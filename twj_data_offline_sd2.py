...
mel_basis_cache = {}
hann_window_cache = {}
def remove_batch_indices(tensor, remove_indices):
    """
    从三维张量中移除指定批次索引的样本
    
    Args:
        tensor: 形状为 (b, t, d) 的张量，b为批次维度
        remove_indices: 要移除的批次索引列表（如 [1, 3, 5]）
        
    Returns:
        filtered_tensor: 移除指定索引后的张量，形状为 (b', t, d)，其中 b' = b - len(remove_indices)
    """
    # 获取批次大小
    ...
    for idx in remove_indices:
        if idx < 0 or idx >= batch_size:
            raise ValueError(f"无效的索引 {idx}，批次大小为 {batch_size}")
    
    # 构造保留掩码：初始为全True，需要移除的索引设为False
    keep_mask = torch.ones(batch_size, dtype=torch.bool, device=tensor.device)
    keep_mask[remove_indices] = False  # 将需要移除的索引标记为False
    
    # 应用掩码过滤张量（只保留True的位置）
    filtered_tensor = tensor[keep_mask]
