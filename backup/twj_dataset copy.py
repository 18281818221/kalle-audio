...
class TTSDataset_online_parquet(Dataset):
    def __init__(self, 
        ...
        from twj_utils import read_parquet
    ...
    def __getitem__(self, idx):
        ...
        # norm_wav = torch.FloatTensor(norm_wav.reshape(1, -1)).to(self.device)
        dual_norm_wav = norm_wav.repeat(2, 1)
        

        # # 修改: 根据通道数动态处理
        # # 转换为张量并转置为 (通道数, 采样点数) 格式
        # dual_norm_wav_tensor = torch.from_numpy(dual_norm_wav.T).float()
        # n_channels = dual_norm_wav_tensor.shape[0]
        
        # # 单通道重复为双通道，双通道直接使用，多通道截取前两通道
        # if n_channels == 1:
        #     wav_processed = dual_norm_wav_tensor.repeat(2, 1)
        # elif n_channels == 2:
        #     wav_processed = dual_norm_wav_tensor
        # else:
        #     # 多通道情况取前两通道
        #     wav_processed = dual_norm_wav_tensor[:2, :]


        # with torch.no_grad():
        #     # 修改4: 使用pretransform.encode替换extract_latents，与main函数保持一致
        #     mean_scale_latent = self.generator.pretransform.encode(dual_norm_wav_tensor.unsqueeze(0))
        ...
        #  添加mel特征生成步骤
        mel = self.mel_transformer(norm_wav) # 1, 80, t
        print('mel', mel.shape)
        # 去掉多余的维度，变为 (80, T)
        mel = mel.squeeze(0)
        T = mel.shape[1]  # 获取总帧数
        ...
        if T >= 200:
            # 随机生成起始索引
            start_idx = torch.randint(0, T - 200 + 1, (1,)).item()
            # 截取连续200帧
            mel_random_200 = mel[:, start_idx:start_idx + 200]
            print('随机截取200帧后的形状:', mel_random_200.shape)  # 输出 (80, 200)
        else:
            # 如果帧数不足200，可以选择填充或其他处理
            print(f"警告：梅尔频谱帧数 {T} 小于200，无法截取")
            # 示例：用0填充到200帧
            mel_random_200 = torch.nn.functional.pad(mel, (0, 200 - T), mode='constant', value=0)
