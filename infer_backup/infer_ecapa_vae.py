...
class Llasa(nn.Module):
    ...
    def infer(
        ...
        if mels is not None :
            ...
            speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_disp.chunk(2,dim=1)
            speaker_cond = torch.randn_like(speaker_cond_mean) * torch.exp(speaker_cond_logs_scale) + speaker_cond_mean
        # elif self.speaker_cond is not None:
        #     speaker_cond = self.speaker_cond
        else:
    ...
    def optim(self,
        ...
        for step in range(max_step):
            ...
            with torch.autocast(device_type="cuda"):
                ...
                with torch.no_grad():
                    self.model.speaker_encoder.eval()
                    speaker_cond = self.model.speaker_encoder(mel.transpose(1,2))
                speaker_cond_disp = self.model.speaker_cond_disp_linear(speaker_cond)
                speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_disp.chunk(2,dim=1)
                speaker_cond = torch.randn_like(speaker_cond_mean) * torch.exp(speaker_cond_logs_scale) + speaker_cond_mean


                input_embed = torch.cat((speaker_cond.unsqueeze(1),text_embed,audio_embed),dim=1)
