...
class infer_tools:
    ...
    def init_output_info(self):
        ...
        if self.opt_before_infer:
            ...
            self.opt_step = 50
            self.target_name += f'-{self.opt_step}'
    ...
    def infer(self):
        ...
        for utt in tqdm(self.utt2target_txt):
            ...
            if self.opt_before_infer:
                ...
                self.optim(input_ids, prompt_mean_scale_latent,max_step=self.opt_step)
...
# _300_90_120_5e-6
    def optim(self,
        ...
        warmup_step = int(max_step * 0.2)
        training_steps = int(max_step * 0.6)
