...
class Llasa_framewise_cond(nn.Module):
    def __init__(
        self,
        config,
        tokenizer,
    ...
    def forward(
        ...
        speaker_cond = self.speaker_encoder(mels.transpose(1,2))
        speaker_cond_disp = self.speaker_cond_disp_linear(speaker_cond)
        speaker_cond_disp = speaker_cond_disp.unsqueeze(1)
        speaker_cond_disp_expand = speaker_cond_disp.expand(-1,t,-1)
        speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_disp_expand.chunk(2,dim=2)
        speaker_cond = torch.randn_like(speaker_cond_mean) * torch.exp(speaker_cond_logs_scale) + speaker_cond_mean

        # speaker_cond,text_embed = self.mrte(mels,text_embed)
        # speaker_cond = self.speaker_encoder(mels.transpose(1,2))    # b,d
        # speaker_cond = speaker_cond.unsqueeze(1)                    # b 1 d
        # speaker_cond_expand = speaker_cond.expand(-1,t,-1)          # b t d

        input_embed = (audio_embed * audio_mask.unsqueeze(-1)) + (text_embed * ids_mask.unsqueeze(-1))   # b,t,d
        attention_mask = ids_mask + audio_mask
        # import pdb;pdb.set_trace()
        # input_embed = torch.concat((speaker_cond.unsqueeze(1),input_embed),dim=1)
        input_embed = input_embed + speaker_cond
    ...
    def infer(
        ...
        t = text_embed.shape[1] + audio_embed.shape[1] if audio_latents is not None else text_embed.shape[1]

        self.speaker_encoder.eval()
        if mels is not None:
            speaker_cond = self.speaker_encoder(mels.transpose(1,2))    # 1 2048
            speaker_cond_disp = self.speaker_cond_disp_linear(speaker_cond) # 1 4096
            
            speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_disp.chunk(2,dim=1)
            speaker_cond_single = speaker_cond_disp

            speaker_cond_disp = speaker_cond_disp.unsqueeze(1)  # 1 1 4096
            speaker_cond_disp_expand = speaker_cond_disp.expand(-1,t,-1) # 1 t 4096
            speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_disp_expand.chunk(2,dim=2) # 1 t 2048
            speaker_cond_framewise = torch.randn_like(speaker_cond_mean) * torch.exp(speaker_cond_logs_scale) + speaker_cond_mean
        
        else:
            zero_mean = torch.zeros((1,t,2048),dtype=text_embed.dtype,device=text_embed.device)
            one_scale = torch.ones((1,t,2048),dtype=text_embed.dtype,device=text_embed.device)
            speaker_cond_framewise = torch.randn_like(zero_mean) * one_scale + zero_mean

            zero_mean = torch.zeros((1,2048),dtype=text_embed.dtype,device=text_embed.device)
            one_scale = torch.ones((1,2048),dtype=text_embed.dtype,device=text_embed.device)
            speaker_cond_single = torch.cat((zero_mean,one_scale),dim=1)



        input_embed = torch.cat((text_embed,audio_embed),dim=1) if audio_latents is not None else text_embed
        input_embed = input_embed + speaker_cond_framewise
        final_audio_latents_lst = []
        ...
        for i in range(max_length):
            ...
            audio_embed = self.audio_linear(audio_latent)           # 1 1 2048
            speaker_cond_mean,speaker_cond_logs_scale = speaker_cond_single.chunk(2,dim=1)
            speaker_cond = torch.randn_like(speaker_cond_mean) * torch.exp(speaker_cond_logs_scale) + speaker_cond_mean
            audio_embed = audio_embed + speaker_cond
...
class infer_tools:
    ...
    def init_model(self):
        model = Llasa_framewise_cond(self.config['model'],self.tokenizer)
    ...
    def infer(self):
        for utt in tqdm(self.utt2target_txt):
            ...
            if self.use_prompt:
                ...
                target_text_tokenized = self.tokenizer.encode(target_text)[1:] if not self.no_icl else self.tokenizer.encode(target_text)
                if self.no_icl:
                    audio_latents = None
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n',action='store_true', help='No icl')
    
    args = parser.parse_args()

    infer = infer_tools(args.config, args.device, args.test_mod, args.check_point_path, args.i, args.s, args.o, args.n)
    infer.infer()