...
from twj_dataset import TTSDataset_online_parquet,DynamicBatchGenerator,dict_to_device, get_lance_filelist
...
def main():
    ...
    train_dataset = TTSDataset_online_lance(
    ...
    # Use PyTorch DataLoader to yield single items; keep dynamic batch assembly via DynamicBatchGenerator
    ...
    data_loader = DataLoader(
        ...
        pin_memory=True,
        collate_fn=lambda x: x[0],  # return the single item instead of a list
    )

    while True:

        for item in tqdm(data_loader, disable=not accelerator.is_local_main_process):
