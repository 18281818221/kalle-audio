...
from aslp.data.base import Base
...
class LanceReader:
    def __init__(self, uri: str, filter_str: str=None, target_cls: Base=Base) -> None:
        self.uri = uri
        self.data_id_cache_file = os.path.join(self.uri,'data_id.pkl')
        self.ds = load(uri)
    ...
    def get_ids(self, ids_col: str="data_id", filter_str: str=None, raw: bool=False, progress: bool=False) -> list[Base]:
        ...
        """
        if not raw and os.path.exists(self.data_id_cache_file):
            print(f"using cache data_id: {self.data_id_cache_file}")
            return pickle.load(open(self.data_id_cache_file, 'rb'))
        
        items = self.ds.to_table([ids_col], filter=filter_str, with_row_id=True).to_pandas()
        if raw:
            return items
        res = self.target_cls.from_pandas(items, progress)
        pickle.dump(res, open(self.data_id_cache_file, 'wb'))
